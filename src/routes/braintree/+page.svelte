			...
			<div class="mx-auto max-w-lg">
				...
				<!-- Checkout Form -->
					<form class="grid gap-y-8" method="POST" use:enhance={ async ({ cancel }) => {
						if (processing || !dropin.isPaymentMethodRequestable()) cancel()
						disabled = true
						processing = true
						// try {
						// 	if (!customer) {
						...
						try {
							...
							dropin.requestPaymentMethod(async function(err, payload) {
								if (err) {
									...
									let result = await client.mutation(AddOrderPayment, { input: { method: 'braintree', metadata: { nonce: payload.nonce } } }).toPromise()
						...
						<section id="address">
							...
							<GooglePlacesAutocomplete apiKey={PUBLIC_GOOGLE_PLACES_API_KEY} />
						...
						{/if}
						<!-- Submission -->         
						<button disabled={processing || disabled} type="submit" class="w-full items-center justify-center rounded-md border border-transparent bg-lime-600 px-5 py-3 text-base font-medium text-white hover:bg-lime-700">
